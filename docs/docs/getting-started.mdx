...
Sparrow runs locally or in a set of Docker
containers.
...
Work is progressing on a "standalone" version of Sparrow that does not require
interacting with the source code. But for now, only development installation
is supported.

If you are planning to run Sparrow on a Windows based operating system, please see [Running Sparrow on Windows](/docs/sparrow-on-windows) before you continue.
...
To install the command-line application, `cd` to the `Sparrow` repository directory
and run the setup command:
```
make install-dev
```
This will build a development version of the
`sparrow` command-line runner and link it into `/usr/local/bin`
(which should be on your `PATH`).
Typing `sparrow` at the command line will download and build containers (this will take a long
...
- `sparrow console` creates a Python shell within the application,
