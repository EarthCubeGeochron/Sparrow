...
:::info New!
Sparrow is now available via a [bundled download](/docs/getting-started#option-1-bundled-version).
:::

Currently there are two options for downloading Sparrow. Either
download a bundled version or clone the codebase directly from Github. The former, bundled version,
is an easier download and startup process although it doesn't allow access the core repository. However,
there is still the ability to customize a Sparrow instance through frontend and backend plugins.

The second option is to clone the Sparrow repository from Github. This process is slightly more involved, however it gives
access to the core codebase. **NOTE:** modifying the main codebase might make it difficult or
impossible to recieve software updates.

## Option 1: Bundled Version

### Shell command

Downloading the bundled version of Sparrow occurs through a command line `curl` command. `curl` commands are used
...
Or you can continute onto to [creating a new user](/docs/getting-started#creating-a-user) and [creating a sparrow configuration](/docs/getting-started#running-a-configured-application).

## Option 2: Cloning Sparrow Codebase
