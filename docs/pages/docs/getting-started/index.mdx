...
The `compose` tool is used to manage the set of Docker containers that comprise
the Sparrow application. As of Sparrow version 3.2.0 (early 2024), `docker-compose` is no longer
bundled with Sparrow, and must be installed separately. See the [Docker compose installation guide](https://docs.docker.com/compose/install/)
