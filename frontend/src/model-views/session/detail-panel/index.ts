...
const AnalysisAttributes = function(props){
  ...
  return h(APIResultView, {route: "/attribute", params: {analysis_id}, placeholder: null }, data=> {
    if (data == null) { return null; }
    if (data.length === 0) { return null; }
    const groupedData = group(data, d => d.parameter);

    return h(Array.from(groupedData, ([k,v]) => h("li.attribute", [
      h('span.parameter', `${k}:`),
      h('ul.values', v.map(d => h('li.value', d.value))
      )
    ])));
});
};
...
const Datum = function(props){
  ...
  return h('li.datum.bp3-text', [
    ...
    h.if(d.error != null)('span.error', [
      ...
      h.if(d.error_metric != null)('span.error-metric', (
        `(${d.error_metric})`
      ))
    ]),
    " ",
...
const AnalysisDetails = function(props){
  ...
  return h(Card, {className: 'analysis-details'}, [
    h.if(a.analysis_type != null)('h3.analysis-type', toTitleCase(a.analysis_type)),
...
const SessionDetails = function(props){
  ...
  if (showTitle == null) { showTitle = false; }
