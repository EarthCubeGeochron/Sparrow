...
const ContentArea = ({data, title, className}) => h('div.content-area', [
  h('h5', [
    h('span.count', data.length),
    " ",
    pluralize(title, data)
  ]),
  h('ul', {className}, data.map(d => h('li', d))
  )
]);
...
const ProjectListComponent = () => h('div.data-view.projects', [
    h(Callout, {
      icon: 'info-sign',
      title: "Projects"
