...
const HomeButton = props => h(LinkButton, {
...
const CatalogNavbar = ({base, ...rest}) => // A standalone navbar for the admin panel, can be enabled by default
...
const AdminMain = props => h(Frame, {id: 'adminBase', ...props}, [
  h('h1', "Catalog"),
  h('div.catalog-index', [
...
const CatalogBody = ({base}) => // Render main body
h(Switch, [
  h(Route, {
...
const Catalog = ({base}) => h('div.catalog', [
  h(LoginSuggest),
  h(CatalogBody, {base})
]);
