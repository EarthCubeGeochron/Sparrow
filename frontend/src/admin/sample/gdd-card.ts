...
const SnippetResult = function(props){
  const {title, pubname, publisher, coverDate,
   authors, highlight, URL} = props;
  return h(LinkCard, {href: URL, target: '_blank', className: 'snippet-result'}, [
    h('h2.title', title),
    ...
    h('h4.date', coverDate),
    h('div.snippets', highlight.map(d => h(Snippet, {html: d})))
  ]);
};
...
const SnippetList = props=> {
  ...
  return h('div.snippet-list', null, items.map(d => h(SnippetResult, d))
  );
};
...
class GeoDeepDiveCard extends Component {
  render() {
    const {sample_name} = this.props;
    const route = 'https://geodeepdive.org/api/v1/snippets';
    const params = {term: sample_name};
    return h(CollapsePanel, {title: "GeoDeepDive results", storageID: 'gdd-results'}, (
      h(ResultsPanel, [
        h(InfoCallout, {
          icon: 'book',
